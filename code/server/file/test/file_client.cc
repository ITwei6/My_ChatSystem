...
TEST(put_test, single_file) {
    //1. 读取当前目录下的指定文件数据
    std::string body;
    ASSERT_TRUE(tew_im::readFile("./Makefile", body));
    ...
    tew_im::PutSingleFileRsp *rsp = new tew_im::PutSingleFileRsp();
...
TEST(get_test, single_file) {
    ...
    brpc::Controller *cntl = new brpc::Controller();
    rsp = new tew_im::GetSingleFileRsp();
    stub.GetSingleFile(cntl, &req, rsp, nullptr);
    ASSERT_FALSE(cntl->Failed());
    ASSERT_TRUE(rsp->success());
    //将文件数据，存储到文件中
    ASSERT_EQ(single_file_id, rsp->file_data().file_id());
    tew_im::writeFile("make_file_download", rsp->file_data().file_content());
}
...
TEST(put_test, multi_file) {
    //1. 读取当前目录下的指定文件数据
    std::string body1;
    ASSERT_TRUE(tew_im::readFile("./base.pb.h", body1));
    std::string body2;
    ASSERT_TRUE(tew_im::readFile("./file.pb.h", body2));
    //2. 实例化rpc调用客户端对象，发起rpc调用
    tew_im::FileService_Stub stub(channel.get());

    tew_im::PutMultiFileReq req;
    ...
    brpc::Controller *cntl = new brpc::Controller();
    tew_im::PutMultiFileRsp *rsp = new tew_im::PutMultiFileRsp();
...
TEST(get_test, multi_file) {
    ...
    brpc::Controller *cntl = new brpc::Controller();
    rsp = new tew_im::GetMultiFileRsp();
    ...
    tew_im::writeFile("base_download_file1",file_data1.file_content());
    auto file_data2 = map[multi_file_id[1]];
    tew_im::writeFile("file_download_file2", file_data2.file_content());
}
...
int main(int argc, char *argv[])
{
    ...
    auto put_cb = std::bind(&tew_im::ServiceManager::onServiceOnline, sm.get(), std::placeholders::_1, std::placeholders::_2);
    auto del_cb = std::bind(&tew_im::ServiceManager::onServiceOffline, sm.get(), std::placeholders::_1, std::placeholders::_2);
    //2. 构造服务发现对象
    tew_im::Discovery::ptr dclient = std::make_shared<tew_im::Discovery>(FLAGS_etcd_host, FLAGS_base_service, put_cb, del_cb);
