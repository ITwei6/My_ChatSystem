...
namespace tew_im
{
    ...
    public:
                    ...
                    const std::string &uid, const std::string &ssid)
        {
            ...
            DEBUG_LOG("新增长连接用户信息：{}-{}-{}", (size_t)conn.get(), uid, ssid);
        }
        server_t::connection_ptr connection(const std::string &uid)
        {
            ...
            {
                ERROR_LOG("未找到 {} 客户端的长连接！", uid);
                return server_t::connection_ptr();
            }
            DEBUG_LOG("找到 {} 客户端的长连接！", uid);
            return it->second;
        }
        bool client(const server_t::connection_ptr &conn, std::string &uid, std::string &ssid)
        {
            ...
            {
                ERROR_LOG("获取-未找到长连接 {} 对应的客户端信息！", (size_t)conn.get());
                return false;
            }
            uid = it->second.uid;
            ssid = it->second.ssid;
            DEBUG_LOG("获取长连接客户端信息成功！");
            return true;
        }
        void remove(const server_t::connection_ptr &conn)
        {
            ...
            if (it == _conn_clients.end())
            {
                ERROR_LOG("删除-未找到长连接 {} 对应的客户端信息！", (size_t)conn.get());
                return;
            }
