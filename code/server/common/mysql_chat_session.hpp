...
namespace tew_im
{
    ...
    public:
        ...
        bool insert(ChatSession &cs)
        {
            ...
            {
                ERROR_LOG("新增会话失败 {}:{}！", cs.chat_session_name(), e.what());
                return false;
            }
            return true;
        }
        ...
        bool remove(const std::string &uid, const std::string &pid)
        {
            ...
            {
                ERROR_LOG("删除会话失败 {}-{}:{}！", uid, pid, e.what());
                return false;
            }
            return true;
        }
        std::shared_ptr<ChatSession> select(const std::string &ssid)
        {
            ...
            {
                ERROR_LOG("通过会话ID获取会话信息失败 {}:{}！", ssid, e.what());
            }
            return res;
        }
        std::vector<SingleChatSession> singleChatSession(const std::string &uid)
        {
            ...
            {
                ERROR_LOG("获取用户 {} 的单聊会话失败:{}！", uid, e.what());
            }
            return res;
        }
        std::vector<GroupChatSession> groupChatSession(const std::string &uid)
        {
            ...
            {
                ERROR_LOG("获取用户 {} 的群聊会话失败:{}！", uid, e.what());
            }
            return res;
        }
