...
namespace tew_im
{
    ...
    public:
        ...
        bool insert(FriendApply &ev)
        {
            ...
            {
                ERROR_LOG("新增好友申请事件失败 {}-{}:{}！", ev.user_id(), ev.peer_id(), e.what());
                return false;
            }
            return true;
        }
        bool exists(const std::string &uid, const std::string &pid)
        {
            ...
            try
            {
                ...
                DEBUG_LOG("{} - {} 好友事件数量：{}", uid, pid, r.size());
                flag = !r.empty();
                trans.commit();
            }
            catch (std::exception &e)
            {
                ERROR_LOG("获取好友申请事件失败:{}-{}-{}！", uid, pid, e.what());
            }
            return flag;
        }
        bool remove(const std::string &uid, const std::string &pid)
        {
            ...
            {
                ERROR_LOG("删除好友申请事件失败 {}-{}:{}！", uid, pid, e.what());
                return false;
            }
            return true;
        }
        ...
        std::vector<std::string> applyUsers(const std::string &uid)
        {
            ...
            {
                ERROR_LOG("通过用户{}的好友申请者失败:{}！", uid, e.what());
            }
            return res;
        }
