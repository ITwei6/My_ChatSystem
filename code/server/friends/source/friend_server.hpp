...
#include "mysql_chat_relation.hpp"       // mysql数据管理客户端封装
...
namespace tew_im
{
    class FriendServiceImpl : public tew_im::FriendService
    {
    public:
        FriendServiceImpl(
            ...
            const std::string &message_service_name) : _es_user(std::make_shared<ESUser>(es_client)),
                                                       _mysql_apply(std::make_shared<FriendApplyTable>(std::dynamic_pointer_cast<odb::mysql::database>(mysql_client))),
                                                       _mysql_chat_session(std::make_shared<ChatSessionTable>(std::dynamic_pointer_cast<odb::mysql::database>(mysql_client))),
                                                       _mysql_chat_session_member(std::make_shared<ChatSessionMemberTable>(std::dynamic_pointer_cast<odb::mysql::database>(mysql_client))),
                                                       _mysql_relation(std::make_shared<RelationTable>(std::dynamic_pointer_cast<odb::mysql::database>(mysql_client))),
                                                       _user_service_name(user_service_name),
                                                       _message_service_name(message_service_name),
                                                       _mm_channels(channel_manager) {}
        ~FriendServiceImpl() {}
        virtual void GetFriendList(::google::protobuf::RpcController *controller,
                                   const ::tew_im::GetFriendListReq *request,
                                   ::tew_im::GetFriendListRsp *response,
        ...
        virtual void FriendRemove(::google::protobuf::RpcController *controller,
                                  const ::tew_im::FriendRemoveReq *request,
                                  ::tew_im::FriendRemoveRsp *response,
                                  ::google::protobuf::Closure *done)
        {
            brpc::ClosureGuard rpc_guard(done);
            // 1. 定义错误回调
        ...
        virtual void FriendAdd(::google::protobuf::RpcController *controller,
                               const ::tew_im::FriendAddReq *request,
                               ::tew_im::FriendAddRsp *response,
                               ::google::protobuf::Closure *done)
        {
            brpc::ClosureGuard rpc_guard(done);
            // 1. 定义错误回调
            ...
            if (ret == true)
            {
                ERROR_LOG("{}- 申请好友失败-两者{}-{}已经是好友关系", rid, uid, pid);
            ...
            if (ret == true)
            {
                ERROR_LOG("{}- 申请好友失败-已经申请过对方好友！", rid, uid, pid);
        ...
        virtual void FriendAddProcess(::google::protobuf::RpcController *controller,
                                      const ::tew_im::FriendAddProcessReq *request,
                                      ::tew_im::FriendAddProcessRsp *response,
                                      ::google::protobuf::Closure *done)
        {
            brpc::ClosureGuard rpc_guard(done);
            // 1. 定义错误回调
            ...
            if (ret == false)
            {
                ERROR_LOG("{}- 没有找到{}-{}对应的好友申请事件！", rid, pid, uid);
            ...
            if (ret == false)
            {
                ERROR_LOG("{}- 从数据库删除申请事件 {}-{} 失败！", rid, pid, uid);
            ...
            if (agree == true)
            {
                ...
                if (ret == false)
                {
                    ERROR_LOG("{}- 新增好友关系信息-{}-{}！", rid, uid, pid);
        ...
        virtual void FriendSearch(::google::protobuf::RpcController *controller,
                                  const ::tew_im::FriendSearchReq *request,
                                  ::tew_im::FriendSearchRsp *response,
                                  ::google::protobuf::Closure *done)
        {
            brpc::ClosureGuard rpc_guard(done);
            // 1. 定义错误回调
        ...
        virtual void GetPendingFriendEventList(::google::protobuf::RpcController *controller,
                                               const ::tew_im::GetPendingFriendEventListReq *request,
                                               ::tew_im::GetPendingFriendEventListRsp *response,
                                               ::google::protobuf::Closure *done)
        {
            brpc::ClosureGuard rpc_guard(done);
            // 1. 定义错误回调
        ...
        virtual void GetChatSessionList(::google::protobuf::RpcController *controller,
                                        const ::tew_im::GetChatSessionListReq *request,
                                        ::tew_im::GetChatSessionListRsp *response,
                                        ::google::protobuf::Closure *done)
        {
            brpc::ClosureGuard rpc_guard(done);
            // 1. 定义错误回调
        ...
        virtual void ChatSessionCreate(::google::protobuf::RpcController *controller,
                                       const ::tew_im::ChatSessionCreateReq *request,
                                       ::tew_im::ChatSessionCreateRsp *response,
                                       ::google::protobuf::Closure *done)
        {
            brpc::ClosureGuard rpc_guard(done);
            // 1. 定义错误回调
            ...
            if (ret == false)
            {
                ERROR_LOG("{} - 向数据库添加会话信息失败: {}", rid, cssname);
            ...
            if (ret == false)
            {
                ERROR_LOG("{} - 向数据库添加会话成员信息失败: {}", rid, cssname);
        ...
        virtual void GetChatSessionMember(::google::protobuf::RpcController *controller,
                                          const ::tew_im::GetChatSessionMemberReq *request,
                                          ::tew_im::GetChatSessionMemberRsp *response,
                                          ::google::protobuf::Closure *done)
        {
            brpc::ClosureGuard rpc_guard(done);
            // 1. 定义错误回调
            ...
            // 2. 从数据库获取会话成员ID列表
            auto member_id_lists = _mysql_chat_session_member->members_id(cssid);
    ...
    private:
                          ...
                          const std::string &cssid, MessageInfo &msg)
        {
            ...
            tew_im::MsgStorageService_Stub stub(channel.get());
            stub.GetRecentMsg(&cntl, &req, &rsp, nullptr);
            if (cntl.Failed() == true)
            {
                ERROR_LOG("{} - 消息存储子服务调用失败: {}", rid, cntl.ErrorText());
                return false;
            }
            if (rsp.success() == false)
            {
                ERROR_LOG("{} - 获取会话 {} 最近消息失败: {}", rid, cssid, rsp.errmsg());
                return false;
            }
                         ...
                         std::unordered_map<std::string, UserInfo> &user_list)
        {
            ...
            if (cntl.Failed() == true)
            {
                ERROR_LOG("{} - 用户子服务调用失败: {}", rid, cntl.ErrorText());
                return false;
            }
            if (rsp.success() == false)
            {
                ERROR_LOG("{} - 批量获取用户信息失败: {}", rid, rsp.errmsg());
                return false;
            }
    ...
    public:
                                   ...
                                   const std::string &message_service_name)
        {
            ...
            DEBUG_LOG("设置用户子服务为需添加管理的子服务：{}", user_service_name);
            DEBUG_LOG("设置消息子服务为需添加管理的子服务：{}", message_service_name);
            auto put_cb = std::bind(&ServiceManager::ServiceOnline, _mm_channels.get(), std::placeholders::_1, std::placeholders::_2);
            auto del_cb = std::bind(&ServiceManager::ServiceOnline, _mm_channels.get(), std::placeholders::_1, std::placeholders::_2);
